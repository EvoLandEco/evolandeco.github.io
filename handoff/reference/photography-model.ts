...
function fields(value: Record<string, unknown>, expected: string[], path: string): void {
...
function choice(value: unknown, allowed: string[], path: string): void {
  if (typeof value !== "string" || !allowed.includes(value)) fail(path, `expected ${allowed.join(" or ")}`);
}
...
function id(value: unknown, path: string): void {
  text(value, path);
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value)) fail(path, "use a stable lowercase slug");
}

function countryCode(value: unknown, path: string): void {
  text(value, path);
  if (!/^[A-Z]{2}$/.test(value)) fail(path, "expected an uppercase two-letter country code");
}
...
function date(value: unknown, path: string): void {
  ...
  text(value, path);
  if (!/^\d{4}(?:-\d{2}(?:-\d{2})?)?$/.test(value)) fail(path, "use YYYY, YYYY-MM or YYYY-MM-DD");
  const [year, month, day] = value.split("-").map(Number);
  if (year < 1 || year > 9999) fail(path, "year is outside the supported range");
  if (month !== undefined && (month < 1 || month > 12)) fail(path, "invalid month");
  if (day !== undefined) {
...
function assertShape(value: unknown): asserts value is PhotographyData {
  ...
  fields(root, ["schemaVersion", "fixtureOnly", "title", "intro", "travelLogStatus", "countries", "albums", "photos"], "photography");
  ...
  choice(root.travelLogStatus, ["unconfirmed", "partial", "complete"], "travelLogStatus");

  list(root.countries, "countries").forEach((entry, index) => {
    const path = `countries[${index}]`;
    const row = object(entry, path);
    fields(row, ["code", "name", "slug", "visibility", "visitConfirmed", "mapMarker"], path);
    countryCode(row.code, `${path}.code`);
    text(row.name, `${path}.name`);
    id(row.slug, `${path}.slug`);
    if (row.slug === "media") fail(`${path}.slug`, "media is reserved for public image assets");
  ...
  list(root.albums, "albums").forEach((entry, index) => {
    ...
    const row = object(entry, path);
    fields(row, ["id", "countryCode", "title", "description", "status", "coverPhotoId", "photoOrder"], path);
    id(row.id, `${path}.id`);
    ...
    list(row.photoOrder, `${path}.photoOrder`).forEach((key, i) => id(key, `${path}.photoOrder[${i}]`));
  });

  list(root.photos, "photos").forEach((entry, index) => {
    const path = `photos[${index}]`;
    const row = object(entry, path);
    fields(row, ["id", "albumId", "status", "sourcePath", "publishApproved", "rights", "alt", "caption", "takenOn", "locationLabel", "image"], path);
    id(row.id, `${path}.id`);
    id(row.albumId, `${path}.albumId`);
    choice(row.status, ["draft", "published"], `${path}.status`);
    text(row.sourcePath, `${path}.sourcePath`);
    if (!row.sourcePath.startsWith("media-private/") || /[\\\u0000]/.test(row.sourcePath) ||
        row.sourcePath.split("/").some(part => part === ".." || part === "." || part === "")) {
      fail(`${path}.sourcePath`, "expected a safe relative path under media-private/");
    }
    ...
    if (row.image !== null) {
      ...
      if (!/^\/photography\/media\/[a-z0-9/-]+\.(webp|avif|jpg|png)$/.test(image.src)) {
        fail(`${path}.image.src`, "expected a local photography derivative path");
      }
...
export function validatePhotography(value: unknown, mode: "production" | "test" = "production"): PhotographyData {
  assertShape(value);
  if (value.fixtureOnly && mode !== "test") fail("fixtureOnly", "synthetic content is prohibited in production");
  unique(value.countries.map(c => c.code), "countries.code");
  unique(value.countries.map(c => c.slug), "countries.slug");
  unique(value.albums.map(a => a.id), "albums.id");
  unique(value.albums.map(a => a.countryCode), "albums.countryCode");
  unique(value.photos.map(p => p.id), "photos.id");
  unique(value.photos.flatMap(p => p.image === null ? [] : [p.image.src]), "photos.image.src");
  if (value.travelLogStatus === "complete" && value.countries.some(c => c.visitConfirmed && c.visibility === "private")) {
    fail("travelLogStatus", "use partial when confirmed countries are omitted from public counts");
  }
  ...
  for (const photo of value.photos) {
    if (!albums.has(photo.albumId)) fail(`photos.${photo.id}.albumId`, "album does not exist");
    if (photo.status === "published") {
      if (!photo.publishApproved || !photo.rights.permissionConfirmed) fail(`photos.${photo.id}`, "published image requires owner approval and permission");
      if (photo.image === null) fail(`photos.${photo.id}.image`, "published image requires a derivative");
      if (!photo.alt.trim()) fail(`photos.${photo.id}.alt`, "published image requires descriptive alt text");
    }
  }
  for (const album of value.albums) {
    const country = countries.get(album.countryCode);
    if (!country) fail(`albums.${album.id}.countryCode`, "country does not exist");
    unique(album.photoOrder, `albums.${album.id}.photoOrder`);
    const ownPhotos = value.photos.filter(p => p.albumId === album.id);
    if (album.photoOrder.length !== ownPhotos.length || ownPhotos.some(p => !album.photoOrder.includes(p.id))) {
      fail(`albums.${album.id}.photoOrder`, "must list every photo in this album exactly once");
    }
    for (const key of album.photoOrder) {
      if (photos.get(key)?.albumId !== album.id) fail(`albums.${album.id}.photoOrder`, "contains a foreign or unknown photo");
    }
    if (album.coverPhotoId !== null && photos.get(album.coverPhotoId)?.albumId !== album.id) {
      fail(`albums.${album.id}.coverPhotoId`, "cover is not a photo in this album");
    }
    if (album.status === "published") {
      if (!country.visitConfirmed) fail(`albums.${album.id}`, "confirm the country visit before publishing its album");
      if (album.coverPhotoId === null || photos.get(album.coverPhotoId)?.status !== "published") {
        fail(`albums.${album.id}.coverPhotoId`, "published album requires a published cover");
      }
      if (!ownPhotos.some(p => p.status === "published")) fail(`albums.${album.id}`, "published album needs a published photograph");
    }
  }
  return value;
}
...
export function getPublicPhotography(value: unknown, mode: "production" | "test" = "production"): PublicPhotography {
  ...
  for (const album of data.albums) {
    if (album.status !== "published" || !eligibleCodes.has(album.countryCode)) continue;
    const country = countriesByCode.get(album.countryCode);
    if (!country) fail(`albums.${album.id}`, "public country relation is missing");
    ...
    for (const photoId of album.photoOrder) {
      ...
      if (photo.image === null) fail(`photos.${photo.id}`, "published derivative is missing");
      publicPhotos.push({
        id: photo.id, href: `${href}/${photo.id}`, alt: photo.alt,
        caption: photo.caption, takenOn: photo.takenOn, locationLabel: photo.locationLabel,
        creator: photo.rights.creator,
        image: { src: photo.image.src, width: photo.image.width, height: photo.image.height },
      });
    }
    const cover = publicPhotos.find(p => p.id === album.coverPhotoId);
    if (!cover) fail(`albums.${album.id}`, "public cover relation is missing");
    albums.push({
      id: album.id, countryCode: country.code, countryName: country.name,
      countrySlug: country.slug, href, title: album.title, description: album.description,
      cover, photos: publicPhotos,
    });
  }
  ...
  return {
    ...
    summary: {
      countryCount: data.travelLogStatus === "unconfirmed" ? null : publicCountries.length,
      countryLabel: data.travelLogStatus === "unconfirmed" ? null :
        data.travelLogStatus === "complete" ? "countries visited" : "countries documented",
      albumCount: albums.length,
      photoCount: albums.reduce((total, album) => total + album.photos.length, 0),
    },
    countries: publicCountries.map(country => ({
      code: country.code, name: country.name,
      mapMarker: country.mapMarker === null ? null : { lat: country.mapMarker.lat, lng: country.mapMarker.lng },
