...
export const TypingAnimation = ({
  ...
  useEffect(() => {
    ...
    if (!hasSequence && (!startOnView || isInView)) {
      startTimeout = setTimeout(() => setDelayComplete(true), delay);
    }
    ...
    >
      <div className="border-border flex flex-col gap-y-2 border-b p-4">
        <div className="flex flex-row items-center gap-x-2">
          <span className="terminal-label mr-auto">software.index</span>
          <div className="h-2 w-2 rounded-full bg-slate-500"></div>
          <div className="h-2 w-2 rounded-full bg-slate-500"></div>
          <div className="h-2 w-2 rounded-full bg-slate-500"></div>
        </div>
      </div>
      <div className="terminal-output" style={{ minHeight: 160 }}>
        {wrappedChildren}
      </div>
    </div>
  );
